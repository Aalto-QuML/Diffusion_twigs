...
def get_sampling_fn(config, noise_scheduler, nodes_dist, batch_size, n_samples, inverse_scaler,
    ...
    if config.sampling.method == 'ancestral':
        ...
        if only_2D:
            ...
            if config.model.name == 'cond_DGT_concat':
                sampler = AncestralSampler(noise_scheduler, time_steps, config.model.pred_data, pred_edge, self_cond,
                                       get_self_cond_fn(config))
            elif config.model.name == 'cond_DGT_twigs':
                if 'multi' in config.exp_type:
                    sampler = AncestralSamplerTwigsMulti(noise_scheduler, time_steps, config.model.pred_data, pred_edge, self_cond, get_self_cond_fn(config))
                else:
                    sampler = AncestralSamplerTwigs(noise_scheduler, time_steps, config.model.pred_data, pred_edge, self_cond, get_self_cond_fn(config))
...
def get_cond_sampling_eval_fn(config, noise_scheduler, nodes_dist, batch_size, n_samples, inverse_scaler,
    ...
    if config.sampling.method == 'ancestral':
        ...
        if config.model.name == 'cond_DGT_concat':
            ...
            sampler = AncestralSamplerTwigs(noise_scheduler, time_steps, config.model.pred_data, pred_edge, self_cond,
                                            get_self_cond_fn(config))
    else:
        raise ValueError('Invalid sampling method!')
    ...
    def sampling_fn(model, classifier):
        ...
        with torch.no_grad():
            ...
            for r in range(num_sampling_rounds):
                ...
                if pred_edge:
                    ...
                    # sampling procedure
                    x_node, x_edge = sampler.sampling(model,
                                                      z, node_mask, edge_mask, edge_z,
                                                      context, context_noise=z_cxt)
                    # postprocessing
                    pos, one_hot, fc, edge_types = post_process(x_node, atom_types, include_fc, node_mask,
                                                                inverse_scaler, x_edge, edge_mask, compress_edge)
                else:
        ...
        # visualize_kde(pred_list, target_list)
        # exit(0)
        os.makedirs('plots/data/', exist_ok=True)
        df = pd.DataFrame({f'pred': pred_list, 'target': target_list})
        df.to_csv(f'plots/data/kde_{n_samples}_{model.module.__class__.__name__[-3:]}_{cond_property}.csv', index=False)
...
def get_cond_multi_sampling_eval_fn(config, noise_scheduler, nodes_dist, batch_size, n_samples, inverse_scaler,
    ...
    cond_prop3 = config.cond_property3
    ...
    if config.nprops == 3:
        mean3, mad3 = prop_norm[cond_prop3]['mean'], prop_norm[cond_prop3]['mad']
    outputNorm = {'mu': 1., 'alpha': 1, 'homo': 1000., 'lumo': 1000., 'gap': 1000, 'Cv': 1.}
    ...
    if config.sampling.method == 'ancestral':
        ...
        if config.model.name == 'cond_DGT_concat':
            ...
            if 'multi' in config.exp_type:
                sampler = AncestralSamplerTwigsMulti(noise_scheduler, time_steps, config.model.pred_data, pred_edge, self_cond,
                                                     get_self_cond_fn(config))
            else:
                sampler = AncestralSamplerTwigs(noise_scheduler, time_steps, config.model.pred_data, pred_edge, self_cond,
                                                get_self_cond_fn(config))
    ...
    def sampling_fn(model, classifier1, classifier2, classifier3, nprops):
        model.eval()
        classifier1.eval()
        classifier2.eval()
        if nprops == 3:
        ...
        with torch.no_grad():
            ...
            for r in range(num_sampling_rounds):
                ...
                if pred_edge:
                    edge_z = sample_symmetric_edge_feature_noise(batch_size, max_n_nodes, edge_nf, edge_mask)
                    # sampling procedure
                    x_node, x_edge = sampler.sampling(model, z, node_mask, edge_mask, edge_z, context, context_noise=z_cxt)
                    # postprocessing
                    pos, one_hot, fc, edge_types = post_process(x_node, atom_types, include_fc, node_mask,
                                                                inverse_scaler, x_edge, edge_mask, compress_edge)
                else:
                ...
                pred2 = classifier2(h0=one_hot.reshape(bs * b_node, -1), x=pos.reshape(bs * b_node, -1), edges=full_edges,
                                    ...
                                    edge_mask=edge_mask, n_nodes=b_node)
                if nprops == 3:
                    pred3 = classifier3(h0=one_hot.reshape(bs * b_node, -1), x=pos.reshape(bs * b_node, -1),
                                        edges=full_edges, edge_attr=None, node_mask=node_mask.reshape(bs * b_node, -1),
                                        edge_mask=edge_mask, n_nodes=b_node)
                ...
                target2 = context[:, 1:2].clone().squeeze(-1)
                target2 = target2 * mad2 + mean2
                pred2 = pred2 * mad2 + mean2

                if nprops == 3:
                    target3 = context[:, 2:3].clone().squeeze(-1)
                    target3 = target3 * mad3 + mean3
                    pred3 = pred3 * mad3 + mean3

                # calculate the l1 loss of output
                MAE1_losses.append(loss_l1(pred1, target1))
                MAE2_losses.append(loss_l1(pred2, target2))
                if nprops == 3:
                    MAE3_losses.append(loss_l1(pred3, target3))
        ...
        if nprops == 3:
            MAE3_losses = torch.cat(MAE3_losses)[:n_samples]
        mean1_loss = MAE1_losses.mean().item()
        mean2_loss = MAE2_losses.mean().item()
        if nprops == 3:
            mean3_loss = MAE3_losses.mean().item()
        else:
            mean3_loss = 0
        return processed_mols[:n_samples], mean1_loss * outputNorm[cond_prop1], mean2_loss * outputNorm[cond_prop2], mean3_loss * outputNorm[cond_prop3]
...
class AncestralSampler:
    ...
    def sampling(self, model, z_T, node_mask, edge_mask, edge_z_T=None, context=None, **kwargs):
        x = z_T
        edge_x = edge_z_T
        bs = z_T.shape[0]
...
class AncestralSamplerTwigsMulti:
    def __init__(self, noise_scheduler, time_steps, model_pred_data, pred_edge=False, self_cond=False,
                 cond_process_fn=None):
        self.noise_scheduler = noise_scheduler
        self.t_array = time_steps
    ...
    def sampling(self, model, z_T, node_mask, edge_mask, edge_z_T=None, context=None, **kwargs):
        ...
        bs = z_T.shape[0]
        cond_x, cond_edge_x = None, None
        p = kwargs.get('context_noise').to(x)
        for i in range(len(self.t_array)):
            t = self.t_array[i]
            s = self.s_array[i]
            alpha_t, sigma_t = self.noise_scheduler.marginal_prob(t)
            ...
            if self.pred_edge:
                if self.self_cond:
                    assert self.model_pred_data
                    pred_t, edge_pred_t, pred_p,p2,p3 = model(vec_t, x, node_mask, edge_mask,
                                                        edge_x=edge_x,
                    ...
                    pred_t, edge_pred_t, pred_p = model(vec_t, x, node_mask, edge_mask,
                                                        edge_x=edge_x,
                                                        noise_level=noise_level,
                                                        context=context,
                                                        context_noise=p)
            ...
            # update context
            n1 = ((alpha_t_given_s * sigma_s ** 2 / sigma_t ** 2).repeat(bs)).unsqueeze(-1)
            n2 = ((alpha_s * sigma2_t_given_s / sigma_t ** 2).repeat(bs)).unsqueeze(-1)
            p_mean = (n1 * p[:,0].unsqueeze(-1)) + (n2 * pred_p.unsqueeze(-1))
            p_mean2 = (n1 * p[:, 1].unsqueeze(-1)) + (n2 * p2.unsqueeze(-1))
            if p3 is not None:
                p_mean3 = (n1 * p[:, 2].unsqueeze(-1)) + (n2 * p3.unsqueeze(-1))
                p3 = p_mean3 + expand_dims(sigma.repeat(bs), p_mean3.dim()) * torch.randn(p_mean3.shape).to(x)
            p = p_mean + expand_dims(sigma.repeat(bs), p_mean.dim()) * torch.randn(p_mean.shape).to(x)
            p2 = p_mean2 + expand_dims(sigma.repeat(bs), p_mean2.dim()) * torch.randn(p_mean2.shape).to(x)
            if p3 is not None:
                p=torch.cat([p,p2,p3],dim=-1).to(x)
            else:
                p = torch.cat([p, p2], dim=-1).to(x)
...
class AncestralSamplerTwigs:
    ...
    def sampling(self, model, z_T, node_mask, edge_mask, edge_z_T=None, context=None, **kwargs):
        ...
        for i in range(len(self.t_array)):
            ...
            n1 = ((alpha_t_given_s * sigma_s ** 2 / sigma_t ** 2).repeat(bs)).unsqueeze(-1)
            n2 = ((alpha_s * sigma2_t_given_s / sigma_t ** 2).repeat(bs)).unsqueeze(-1)
            p_mean = (n1 * p) + (n2 * pred_p.unsqueeze(-1))
            p = p_mean + expand_dims(sigma.repeat(bs), p_mean.dim()) * torch.randn(p_mean.shape).to(x)

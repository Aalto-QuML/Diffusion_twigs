...
def get_dataset(config, transform=True):
    ...
    if transform:
        ...
        if name_transform == 'Edge':
            ...
            transform = EdgeComCondMultiTransform(dataset_info['atom_encoder'].values(), config.data.include_aromatic,
                                                  ...
                                                  prop2idx[config.cond_property3], config.nprops)
        else:
    ...
    if 'cond' in config.exp_type:
        ...
        first_train_dataset = dataset.index_select(split_idx['first_train']) if cuda else dataset.index_select(dataset.indices()[:20])
        second_train_dataset = dataset.index_select(split_idx['second_train']) if cuda else dataset.index_select(dataset.indices()[20:40])
        val_dataset = dataset.index_select(split_idx['valid']) if cuda else dataset.index_select(dataset.indices()[40:50])
        test_dataset = dataset.index_select(split_idx['test']) if cuda else dataset.index_select(dataset.indices()[50:60])
...
class EdgeComCondMultiTransform(object):
    ...
    def __init__(self, atom_type_list, include_aromatic, property_idx1, property_idx2, property_idx3, nprops):
        super().__init__()
        self.atom_type_list = torch.tensor(list(atom_type_list))
        self.include_aromatic = include_aromatic
    ...
    def __call__(self, data: Data):
        ...
        properties = data.y
        prop_list2 = [self.property_idx1, self.property_idx2]
        prop_list3 = [self.property_idx1, self.property_idx2, self.property_idx3]
        prop_list = prop_list2 if self.nprops == 2 else prop_list3
        property_data = []

        ...
        if config.only_2D:
            ...
            if config.model.name == 'cond_DGT_concat':
                ...
                if 'multi' in config.exp_type:
                    loss_fn = get_sde_graph_loss_fn_twigs_multi(noise_scheduler, train, scaler, config, prop_dist)
                else:
                    loss_fn = get_sde_graph_loss_fn_twigs(noise_scheduler, train, scaler, config, prop_dist)
    else:
...
def get_sde_graph_loss_fn_twigs_multi(noise_scheduler, train, scaler, config, prop_norm=None):
    ...
    def loss_fn(model, batch):
        ...
        noise = sample_combined_position_feature_noise(xh.shape[0], xh.shape[1], xh.shape[2] - 3, node_mask)
        edge_noise = sample_symmetric_edge_feature_noise(edge_x.shape[0], edge_x.shape[1], edge_x.shape[-1], edge_mask)
        context_noise = torch.randn(context.shape)
        ...
        if self_cond:
            ...
            if random() < 0.5:
                with torch.no_grad():
                    cond_x, cond_edge_x, _,_,_ = model(t, z_t, node_mask,
                                                edge_mask,
                                                edge_x=edge_z_t,
            ...
            pred, edge_pred, c_pred, c2,c3 = model(t,
                                            z_t,
                                            node_mask,
                                            edge_mask,
                                            edge_x=edge_z_t,
        ...
        if pred_data:
            # data prediction loss
            losses_prop = torch.square(c_pred - context[:,0])
            losses_prop = torch.sum(losses_prop, dim=-1)
            losses_prop2 = torch.square(c2 - context[:,1])
            losses_prop2 = torch.sum(losses_prop2, dim=-1)
            if config.nprops == 3:
                losses_prop3 = torch.square(c3 - context[1, 2])
                losses_prop3 = torch.sum(losses_prop3, dim=-1)
            # losses_prop = torch.mean(losses_prop, dim=-1)
            losses_pos = torch.square(pred[:, :, :3] - align_pos)
            losses_pos = torch.mean(losses_pos, dim=-1)
            losses_pos = torch.sum(losses_pos, dim=-1)
            atom_type_pred = pred[:, :, 3:]
        ...
        losses += losses_prop * 0.25
        losses += losses_prop2 * 0.25
        if config.nprops == 3:
            losses += losses_prop3 * 0.25
...
def get_sde_graph_loss_fn_twigs(noise_scheduler, train, scaler, config, prop_norm=None):
    """The loss function for node features, positions and edge features"""
    ...
    def loss_fn(model, batch):
        ...
        if self_cond:
            ...
            if random() < 0.5:
                with torch.no_grad():
                    cond_x, cond_edge_x, _ = model(t, z_t, node_mask,
                                                edge_mask,
                                                edge_x=edge_z_t,
                                                noise_level=noise_level,
                                                cond_x=cond_x,
        ...
        if pred_data:
            # data prediction loss
            losses_prop = torch.square(c_pred - context)
            # losses_prop = torch.mean(losses_prop, dim=-1)
            losses_prop = torch.sum(losses_prop, dim=-1)
            losses_pos = torch.square(pred[:, :, :3] - align_pos)
            losses_pos = torch.mean(losses_pos, dim=-1)
...
def get_sde_graph_loss_fn(noise_scheduler, train, scaler, config, prop_norm=None):
    ...
    def loss_fn(model, batch):
        ...
        if self_cond:
            ...
            if random() < 0.5:
                with torch.no_grad():
                    cond_x, cond_edge_x = model(t, z_t, node_mask,
                                                edge_mask,
                                                edge_x=edge_z_t,
                                                noise_level=noise_level,
                                                cond_x=cond_x,

...
class PoolEquivariantMixBlock(nn.Module):
    def __init__(self, node_dim, edge_dim, time_dim, num_extra_heads, num_heads, cond_time, dist_gbf, softmax_inf,
        ...
        super().__init__()
        self.eqmb = EquivariantMixBlock(node_dim, edge_dim, time_dim, num_extra_heads, num_heads, cond_time, dist_gbf,
                                        softmax_inf, mlp_ratio, act, dropout, gbf_name, trans_name)

        self.node_dec = nn.Sequential(nn.Linear(node_dim, node_dim),
                                      act,
                                      nn.Linear(node_dim, node_dim))

        self.graph_dec = nn.Sequential(nn.Linear(node_dim, node_dim),
                                       act,
                                       nn.Linear(node_dim, 1))
    def forward(self, pos, h, edge_attr, edge_index, node_mask, extra_heads, node_time_emb=None, edge_time_emb=None):
        h, _, _ = self.eqmb(pos, h, edge_attr, edge_index, node_mask.reshape(-1, 1), extra_heads, node_time_emb, edge_time_emb)
        h = self.node_dec(h)
        h = h.view(node_mask.shape[0], node_mask.shape[1], -1)
        h = h * node_mask
        h = torch.sum(h, dim=1)
...
@utils.register_model(name='cond_DGT_twigs')
class CondDgtConcatCls(nn.Module):
    """DGT with pooling """
    def __init__(self, config):
        super().__init__()
        ...
        self.cls = PoolEquivariantMixBlock(hidden_dim, edge_hidden_dim, time_dim, n_extra_heads,
                                           n_heads, cond_time, dist_gbf, softmax_inf, mlp_ratio=mlp_ratio,
                                           dropout=dropout, gbf_name=gbf_name)
        self.cls2 = PoolEquivariantMixBlock(hidden_dim, edge_hidden_dim, time_dim, n_extra_heads,
                                           n_heads, cond_time, dist_gbf, softmax_inf, mlp_ratio=mlp_ratio,
                                           dropout=dropout, gbf_name=gbf_name)
        self.nprops = config.nprops
        if config.nprops == 3:
            self.cls3 = PoolEquivariantMixBlock(hidden_dim, edge_hidden_dim, time_dim, n_extra_heads,
                                                n_heads, cond_time, dist_gbf, softmax_inf, mlp_ratio=mlp_ratio,
                                                dropout=dropout, gbf_name=gbf_name)
        ...
        if cond_time:
            ...
            # self.noise_c_map = nn.Sequential(
            #     sinu_pos_emb,
            #     nn.Linear(learned_dim + 1, time_dim),
            #     nn.GELU(),
            #     nn.Linear(time_dim, time_dim)
        ...
        # Conditional MLP for evolving labels
        self.cond_mlp2 = nn.Sequential(
            nn.Linear(config.nprops, hidden_dim),
            nn.GELU(),
            nn.Linear(hidden_dim, hidden_dim)
        )
        self.cond_lin2 = nn.Linear(cond_ch * hidden_dim // config.nprops, time_dim)
    ...
    def forward(self, t, xh, node_mask, edge_mask, context=None, *args, **kwargs):
        ...
        if self.cond_time:
            noise_level = kwargs['noise_level']
            context_noise = kwargs.get('context_noise')
            # cn_emb = self.noise_c_map(context_noise)
            context_noise = self.cond_lin2(self.cond_mlp2(context_noise).reshape(bs, -1))
            time_emb = self.time_mlp(noise_level) + context + context_noise  # [B, hid_dim*4]
            node_time_emb = time_emb.unsqueeze(1).expand(-1, n_nodes, -1).reshape(bs * n_nodes, -1)
            edge_batch_id = torch.div(edge_index[0], n_nodes, rounding_mode='floor')
            edge_time_emb = time_emb[edge_batch_id]
        else:
        ...
        # classification
        context_pred = self.cls(pos, h, edge_attr, edge_index, node_mask, extra_adj, node_time_emb, edge_time_emb)
        context_pred2 = self.cls2(pos, h, edge_attr, edge_index, node_mask, extra_adj, node_time_emb, edge_time_emb)
        if self.nprops == 3:
            context_pred3 = self.cls3(pos, h, edge_attr, edge_index, node_mask, extra_adj, node_time_emb, edge_time_emb)
        else:
            context_pred3 = None
        ...
        return torch.cat([pos, atom_pred], dim=2), edge_final, context_pred, context_pred2, context_pred3

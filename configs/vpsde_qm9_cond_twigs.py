"""Training Conditional Twigs with single property on QM9"""
...
def get_config():
    ...
    data.num_workers = 16 if cuda else 0
    ...
    # model.name = 'cond_DGT_concat'  # jodo paper
    model.name = 'cond_DGT_twigs'  # our paper
    ...
    # # model2, could requires work for multiple optimizers etc
    # model2 = model.copy_and_resolve_references()
    # model2.name = 'cond_DGT_twigs'
    # config.model2 = model2

    # training
    config.training = training = ml_collections.ConfigDict()
    training.reduce_mean = False
    training.batch_size = 128 if cuda else 16
    training.eval_batch_size = 128 if cuda else 16
    training.eval_samples = 128 if cuda else 16
    training.log_freq = 500 if cuda else 1

    training.n_iters = 2000000 if cuda else 1
    training.snapshot_freq = 50000 if cuda else 1
    # # store additional checkpoints for preemption (meta)
    training.snapshot_freq_for_preemption = 10000 if cuda else 1
    ...
    optim.beta1 = 0.9
    optim.eps = 1e-8
    optim.warmup = 100000 if cuda else 1
    optim.grad_clip = 10.
    ...
    sampling.steps = 1000 if cuda else 2
    sampling.vis_row = 4
    sampling.vis_col = 4
    ...
    evaluate.batch_size = 2500 if cuda else 1  # generation batch size
    evaluate.num_samples = 10000 if cuda else 1  # number of samples for evaluation
    evaluate.begin_ckpt = 40 if cuda else 1
    evaluate.end_ckpt = 40 if cuda else 1

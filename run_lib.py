...
def vpsde_edge_train(config, workdir):
    ...
    fcd_metric = get_fcd_metric(test_mols, n_jobs=16 if torch.cuda.is_available() else 1, device=config.device, batch_size=1000)
...
def vpsde_edge_evaluate(config, workdir, eval_folder="eval"):
    ...
    mose_metric = get_moses_metrics(test_mols, n_jobs=16, device=config.device)
...
def vpsde_train(config, workdir):
    ...
    EDM_metric_2D = get_2D_edm_metric(dataset_info)
    test_mols = [test_ds[i].rdmol for i in range(len(test_ds))]
    fcd_metric = get_fcd_metric(test_mols, n_jobs=16, device=config.device, batch_size=1000)
...
def vpsde_edge_cond_train(config, workdir):
    ...
    # Resume training when intermediate checkpoints are detected
    state = restore_checkpoint(checkpoint_meta_dir, state, config.device)
    initial_step = int(state['step']) if torch.cuda.is_available() else 0
    # initial_step = 0
    print(f'init {initial_step}')
    print(f'mod {config.model.name} prop {config.cond_property}')
    num_train_steps = config.training.n_iters
...
def vpsde_edge_cond_evaluate(config, workdir, eval_folder="eval"):
    ...
    EDM_metric = get_edm_metric(dataset_info, train_mols)
    EDM_metric_2D = get_2D_edm_metric(dataset_info, train_mols)
    mose_metric = get_moses_metrics(test_mols, n_jobs=16 if torch.cuda.is_available() else 1, device=config.device)
    if config.eval.sub_geometry:
        sub_geo_mmd_metric = get_sub_geometry_metric(test_mols, dataset_info, config.data.root)
...
def vpsde_edge_cond_multi_train(config, workdir):
    ...
    prop2idx_sub = {
        ...
        config.cond_property3: prop2idx[config.cond_property3]
    } if config.nprops == 3 else {
        config.cond_property1: prop2idx[config.cond_property1],
        config.cond_property2: prop2idx[config.cond_property2],
    }
    prop_norms = val_ds.compute_property_mean_mad(prop2idx_sub)
...
def vpsde_edge_cond_multi_evaluate(config, workdir, eval_folder="eval"):
    ...
    property_path3 = os.path.join(config.data.root, 'property_classifier', f'evaluate_{config.cond_property3}')
    classifier_path3 = os.path.join(property_path3, 'best_checkpoint.npy')
    args_classifier_path3 = os.path.join(property_path3, 'args.pickle')
    if config.nprops == 3:
        classifier3 = get_classifier(classifier_path3, args_classifier_path3).to(config.device)
    ...
    for ckpt in ckpts:
        ...
        if config.eval.enable_sampling:
            logging.info('Sampling -- ckpt: %d' % (ckpt,))
            # Wrapper EDM sampling
            processed_mols, MAE1_loss, MAE2_loss, MAE3_loss = sampling_fn(model, classifier1, classifier2, classifier3, config.nprops)
            logging.info(f"{config.cond_property1} MAE: %.4f" % MAE1_loss)
            logging.info(f"{config.cond_property2} MAE: %.4f" % MAE2_loss)
            logging.info(f"{config.cond_property3} MAE: %.4f" % MAE3_loss)
